...
export async function createManufacturingDemoProject(): Promise<CreateManufacturingDemoResult> {
  ...
  // 2. Firma Profili (Marmara Endüstriyel Sistemler A.Ş.)
  await db.execute(
  ...
  // 3. İş Fonksiyonları (20 Aktif Fonksiyon)
  ...
  // 5. Takip Bayrakları (Section 5 Açık Konular)
  const followupsData = [
    {
  ...
  // 10. Yönetişim Katmanı (Governance Objects, Subjects, Scopes, Responsibilities, Limits, Authorizations, SoD Risks)
  ...
  // Subjects (Departmanlar ve Roller)
  const govSubjects = [
  ...
  // Sorumluluk Atamaları (Data Owner, Data Steward, Custodian)
  ...
  for (const r of responsibilities) {
    if (objectIdMap[r.objCode] && subjectIdMap[r.subName]) {
      await createGovernanceResponsibility({
        analysis_project_id: projectId,
        governance_object_id: objectIdMap[r.objCode],
        subject_id: subjectIdMap[r.subName],
        scope_id: scopeIdMap[r.scope] || null,
  ...
  // Yetki Matrisi ve Sapmalar (Authorizations with 2 discrepancies)
  ...
  for (const a of authorizations) {
    if (objectIdMap[a.objCode] && subjectIdMap[a.subName]) {
      await createGovernanceAuthorization({
        analysis_project_id: projectId,
        governance_object_id: objectIdMap[a.objCode],
        subject_id: subjectIdMap[a.subName],
  ...
  for (const l of limits) {
    if (objectIdMap[l.objCode] && subjectIdMap[l.subName]) {
      await createGovernanceLimit({
        analysis_project_id: projectId,
        governance_object_id: objectIdMap[l.objCode],
  ...
  // Görevler Ayrılığı (SoD) Riskleri (2 Adet)
  const sodRisks = [
    {
